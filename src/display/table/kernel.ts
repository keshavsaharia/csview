...
export function displayValue(value: number, dir: number[]) {
	for (const displayKernel of displayKernels) {
		const pattern = displayKernel[0]
		const match = pattern & value
		if (match > 0) {
			const kernels = displayKernel[1]
			const matchKernel = kernels.find((kernel) => kernelMatch(kernel, dir))
			if (matchKernel)
...
function kernelMatch(kernel: DisplayKernel, dir: number[]): boolean {
	const kernelDir = kernel[1]
	if (kernelDir.length == 0)
		return true

	return kernelDir.every((value, index) => (
		// If zero, corresponding direction must also be zero
		// Any corresponding direction specification must match
		value == 0 ? dir[index] == 0 : ((value & dir[index]) > 0)))
}

export const displayKernels: DisplayKernels[] = [
	// Rounded corners
	[ ROUNDED, [
